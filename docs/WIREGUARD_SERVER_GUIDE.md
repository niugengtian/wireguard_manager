...
2. 客户端配置的 `AllowedIPs` 是该设备送入隧道的目标路由，可以按设备不同；多条 CIDR 使用英文逗号分隔，例如 `10.255.77.0/24, 172.31.0.0/16`。`0.0.0.0/0` 表示所有 IPv4 流量走隧道，只能在 NAT、转发、DNS 和恢复通道均已验证时使用。修改后，Web 用户点一次 reset，系统先完整交付文件，再在短暂宽限期后自动热切换 Peer，避免下载完成前撤销当前传输链路。
